...
def test_chunk_reply_splits_sentences():
    chunks = chunk_reply_for_tts("Namaste. Main aapki madad kar sakta hoon. Kya aap sun pa rahe hain?")
    assert len(chunks) == 3
...
@pytest.mark.asyncio
async def test_brain_ws_turn_emits_chunk_flow_class_done():
    async with lifespan(app):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            async with client.websocket_connect("/ws/brain") as ws:
                await ws.send_json(
                    {
                        "type": "session_start",
                ...
                await ws.send_json({"type": "session_end", "session_id": "sess-eb6-1"})
...
@pytest.mark.asyncio
async def test_brain_ws_opener_allows_empty_transcript():
    async with lifespan(app):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            async with client.websocket_connect("/ws/brain") as ws:
                ...
                await ws.send_json({"type": "session_end", "session_id": "sess-opener"})

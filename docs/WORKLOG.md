...
_Started 09 Aug 2026. Implementer: Cursor ù Reviewer: Claude ù Sign-off: Nitish._
...
NO commit/deploy/live-call without sign-off ù every claim carries evidence (log line, test name, stash-proof) ù clean builds only, /version verified, no docker-cp, no sed-on-.env ù found a bug outside scope ? RECORD in tracker, never fix silently ù dead air is the worst failure ù STOP at every ? checkpoint.
...
**Real branch points total: 12.** All 12 are profile-able with new `TenantRuntimeProfile` fields. See `docs/REPO_CONTEXT.md` ù3.
...
**Scope:** 12 real branch points from `REPO_CONTEXT.md` ù3.
...
**Counts:** PROFILE-FIELD 5 (#1,#4,#5,#6,#7) ù TEST-SHIM QUARANTINE 6 (#2,#3,#8-12) ù LEAK-PATH 1 (`force_flow` injection ? ù3). **Total new profile fields: 11** (5 profile + 6 test-shim). **One new guard.**
...
**Guard recommendation (audit-only, NOT applied in A2):** add a tenant-catalog check at `turn.py:1007-1009` ? `if catalog is None or forced_flow in catalog: state.flow_stack.append(...)`. This is the **one new guard** from ù1. Closes Leak Path A. Paths B & C already mitigated by `catalog_mode=true`.
...
**4.1 New `TenantRuntimeProfile` fields (11):** `supports_committed_date_coercion` (bool), `timing_slot_set` (list), `ltl_enforce_enabled` (bool), `identity_bypass_flows` (list), `test_agent_id` (str), `test_borrower_id` (str), `test_borrower_factory` (callable), `test_loan_keys` (list), `test_scenario_override_slot` (str), `allow_sot_test_mode` (bool), implicit `has_tenant_config` (bool computed). See `TENANCY_AUDIT.md` ù4.1 for SOT/PLO values + branch-point mapping.
...
| DT | 0% [ ] | 0% [P] (planned) | Folded into A2 (ù4) |
...
Entry #004's em-dashes (?) were double-encoded to `ù?"` by PowerShell `Add-Content`
...
added to `docs/IMPLEMENTATION_TRACKER_V2.md` ùKNOWN-RED TEST FAILURES. Breakdown:
...
- `Collection/PAISALO_FRAGMENT_LIBRARY_V1.md`: new ùH "Dead-air apology (W1-B)"
...
(fragment library ùH candidate #55); hot-swappable via the profile YAML
without a redeploy.
...
**Fragment candidate #56** (`PAISALO_FRAGMENT_LIBRARY_V1.md` ùI,
...
- `PAISALO_FRAGMENT_LIBRARY_V1.md` ? ùI vulnerability de-escalation candidate #56 (PENDING-CLIENT-APPROVAL).
...
- C1 de-escalation script + C4 third-party scripts remain
  PENDING-CLIENT-APPROVAL (fragment library candidates #56 + ùI/ùJ);
...
t1 m2e=0 (engine 159ms) ó same go-server log anomaly class as 2f8f9f01 t8.
...
Same store we already write (InMemory / Upstash / Composite). Compact `sessions:{borrower_id}` JSON list ó not a new table. Session record now carries call_id, ts, disposition, ptp_date, last_ptp_date, last_disposition, last_call_ts, attempts_today.
...
last_ptp_date in the future + campaign dials today -> ptp_reminder ("???? {ptp_date} ?? ?? ??? ???? ??Ö"), no collect, disposition PTP_REMINDED, end_call. Wins over repeat-call when both apply.
...
CommandParseResult.related + ack_text. Register "?? ???? Ö ?? ???? ???" /
...
postgres/memory store ó real reads, no hangup/actions. Simulate forbidden
when CARRIER=asterisk (startup LiveConfigError, same class as ASR/TTS).

### 2. Contract

docs/TOOLS_API_CONTRACT.md ó GET /v1/borrower_state (loan_ref|phone) ->
...
reference. That NPA snapshot compose was a false positive ‚Äî NPA awaits
...
which only lived on the log_turn_decision payload ‚Äî cells looked empty
